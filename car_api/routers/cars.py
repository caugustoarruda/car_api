...
from car_api.schemas.cars import CarSchema, CarPublicSchema, CarListPublicSchema, CarUpdateSchema
...
@router.post(path='/', status_code=status.HTTP_201_CREATED, response_model=CarPublicSchema, summary='Criar novo carro')
async def create_car(car: CarSchema, db: AsyncSession = Depends(get_session)):
    ...
    if plate_exists:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail='Placa já esta em uso'
        )
    
    brand_exists = await db.scalar(select(exists().where(Brand.id == car.brand_id)))
    if not brand_exists:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail='Marca não encontrada'
        )
    ...
    result = await db.execute(select(Car).options(selectinload(Car.brand), selectinload(Car.owner)).where(Car.id == db_car.id))
